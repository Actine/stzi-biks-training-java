...
//      System.out.println(emptyConstInt);       // видасть помилку
        ...
        // Важлива замітка: у Java немає беззнакових числових типів. Половина числових значень виділяється на від’ємні
        // числа, наприклад, якщо byte може мати 2^8 = 256 значень, то його діапазон - від -128 до 127
        ...
        // Об’єкти усіх інших класів крім String треба створювати явно, наприклад за допомогою оператора new (новий):
...
//      someNull.equals(null);      // програма вилетить із помилкою NullPointerException
        ...
        // НАОСТАНОК
        // Перетворення рядків у примітиви, примітивів у рядки

        // Дуже часто нам потрібно буде перетворити рядок (наприклад, те, що вводить користувач) на число.
        // Для цього у класів-обгорток є методи .parse*(String):
        ...
        // Якщо потрібно одразу клас-обгортку, вкористовуйте статичний метод .valueOf(String):
        Integer parsedInt2 = Integer.valueOf("123");
        Double parsedDouble2 = Double.valueOf("123.456");

        // Для перетворення примітивів на рядки, у їх класів-обгорток існують відповідні статичні методи toString():
        String str;
        str = Integer.toString(123);           // "123"
        str = Integer.toBinaryString(123);     // "1111011"
        str = Double.toString(123.456);        // "123.456"
        str = Boolean.toString(true);          // "true"

        // Або, як варіант, можна користуватися методом String.valueOf() для будь-яких типів:
        ...
        str = new Object().toString();         // для інших об’єктів - якесь їх рядкове представлення
                                               // (за замовчуванням - назву класу і хеш-код об’єкта, унікальний для
