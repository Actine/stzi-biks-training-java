    ...
    public static void main(String[] args) {
        ...
        switch (die2) {
            ...
            case 4:
                ...
                break;
            case 2:
            case 3:         // Таким чином можна об’єднувати кілька варіантів
        ...
        // Як це розуміти?
        // Для значення switch перевіряється його рівність послідовно кожному зі значень case.
        ...
        switch (die1) {
            case 1:
                System.out.println("На першому кубику випало 1");
            case 2:
                System.out.println("На першому кубику випало 1 або 2");
            case 3:
                System.out.println("На першому кубику випало 1 або 2, або 3");
            case 4:
                System.out.println("На першому кубику випало 1 або 2, або 3, або 4");
            case 5:
                System.out.println("На першому кубику випало 1 або 2, або 3, або 4, або 5");
            case 6:
                System.out.println("На першому кубику випало 1 або 2, або 3, або 4, або 5, або 6");
        }
...
//          int definedInMethodScope = 30;          // не можна - така змінна уже є в методі
        ...
        // У Java 5 і вище є ще один вид циклу - спрощений цикл for (ще його називають foreach - «для кожного»).
        // Він може використовуватись лише для масивів і колекцій (колекції ми ще не розбирали - там матеріал складний,
        // і це буде згодом), і дозволяє «пробігтися» по всіх елементах масиву чи колекції.
        // Синтаксис: for (<тип> <ім’я змінної> : <масив або колекція>) { ... }
        // де змінна дорівнюватиме кожному наступному елементу масива або колекції на кожній наступній ітерації

        // Приклад: вивести всі числа із масиву
        int[] numbers = {2, 5, 3, 4, 2, 3, 7, 0};

        // Звичайним for-циклом це робиться так:
        for (int j = 0; j < numbers.length; j++) {
            System.out.println(numbers[j]);
        }

        // А спрощеним циклом foreach - так:
        for (int number : numbers) {          // для кожного цілого числа number із масиву numbers
            System.out.println(number);       // вивести цей number
        }

        // -------------------------------------------------------------------------------------------------------------

        // Іще два контролюючі оператори, які використовуються разом із циклами - це break і continue.
        // Оператор break (англ. «зламати», тут - «вирватись», «розірвати») використовується для миттевого виходу
        // із циклу, а оператор continue (англ. «продовжити») - для миттєвого закінчення даної ітерації і переходу
        // на наступну, якщо така буде.

        // Приклад: перевірити, що ВСІ числа у масиві - парні.

        // Введемо змінну, в яку запишемо результат перевірки. Припустимо, що всі числа - парні.
        boolean areAllEven = true;

        // Для наочності порахуємо кількість ітерацій, які пройшов цикл
        int iterations = 0;

        // Перевіряємо числа по черзі, але якщо трапиться хоч одне непарне - немає сенсу перевіряти далі
        for (int number : numbers) {
            iterations++;

            if (number % 2 == 1) {         // Якщо число непарне (ділиться на 2 із остачею 1)
                areAllEven = false;        // запишемо, що перевірка не пройшла
                break;                     // і зупинимо цикл, адже нема сенсу перевіряти наступні елементи
            }
        }
        System.out.println(iterations);    // 2 ітерації - адже вже другий елемент виявився непарним

        // Для continue важко придумати простий приклад - цей оператор є сенс використовувати тоді, коли у циклі
        // присутньо багато логіки, але є умова, за якої ітерацію варто пропустити.

        String[] names = {"Мартін", null, "Марк", "Андрій", null, "Володимир",
                "Владислава", "Сергій", "Василь", null, "Данило", "Дмитро", "Іван"};     // ті, хто був на парі 24.09

        // Для кожного рядка виконати складну операцію, ігноруючи пусті елементи масиву
        for (String name : names) {
            if (name == null) {
                continue;        // Якщо ім’я - null, не виконувати решту ітерації, а перейти одразу до наступного імені
            }
        ...
        // Звісно, якщо інструкцій небагато, це ж саме набагато легше написати без continue, а з прямою умовою:
        for (String name : names) {
            if (name != null) {
                System.out.println(name);
            }
        ...
        // Золоте правило - чим менше коду у вкладених блоках, тим легше його читати.
        // Ним і треба керуватися при рішенні, використовувати трюк із continue чи ні.

        // -------------------------------------------------------------------------------------------------------------

        // Цикл можна зробити нескінченним - для цього достатньо, щоб ніколи не настала умова виходу із циклу
        // (тобто, щоб умова циклу while, do...while або for ніколи не повертала false).

        // Нескінченні цикли можуть бути корисними тільки за одної умови - якщо із них все ж таки можна забезпечити
        // вихід (за допомогою оператора break, або return для виходу із метода, або System.exit() для зупинки всієї
        // програми, або викидання помилки). Інакше програма ЗАЦИКЛИТЬСЯ, їстиме ресурси процесора, розігріватиме вам
        // комп’ютер і не зупиниться аж доки ви її не вб’єте, вона не зжере усю пам’ять, або ви не вимкнете комп’ютер.

        // Найпоширеніше використання нескінченного циклу - нескінченний цикл while замість циклу do...while,
        // наприклад для обробки вводу користувача.

        System.out.println("Знову кидаємо третій кубик, доки не випаде таке число, як на першому:");

        while (true) {                                        // Повторювати, доки true == true (тобто, вічно)
            die3 = random.nextInt(6) + 1;
            System.out.println("Випало значення " + die3);

            if (die3 == die1) {                               // Але якщо значення співпали -
                System.out.println("Значення співпали!");     // вивести повідомлення
                break;                                        // і вийти із циклу
            }
        }

        // Цикл for можна зробити нескінченним, просто залишивши усі три інструкції пустими
        for ( ; ; ) {
            // тіло циклу, посередині якогось десь має бути:
            break;
        }

        // -------------------------------------------------------------------------------------------------------------

        // І наостанок, пам’ятайте, що режим зневаждення (debug, рос. «отладка») і підказки ідеївського інспектора
        // (виділені жовтим шматки коду з підказками) - ваші друзі!
